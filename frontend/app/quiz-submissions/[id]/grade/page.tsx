...
export default function GradeQuizPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const submissionRes = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/quiz-submissions/${submissionId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleSaveGrade = async () => {
    ...
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/quiz-submissions/${submissionId}/grade`,
        {
          score,
          remarks,
        },
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-600 mt-1">
                  {submission.quiz_title} - {submission.course_title}
                </p>
              </div>
            </div>
                ...
                <div className="bg-white rounded-lg border border-slate-200 p-6 space-y-6">
                  ...
                  {answers.length > 0 ? (
                    answers.map((answer, idx) => (
                      <div
                        key={answer.id}
                        className="pb-6 border-b border-slate-200 last:pb-0 last:border-0"
                      >
